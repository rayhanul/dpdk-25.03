 ...
 *
 * The Broadcom BCM2711 (Raspberry Pi 4 / Compute Module 4) is the common case:
 * with 4GB or more of RAM the inbound viewport cannot start at 0 (the outbound
 * window already sits at 3GB in PCIe space), so the firmware places system
 * memory at PCIe address 0x4_0000_0000.  Without the translation every DPDK
 * descriptor ring and mbuf address handed to the NIC is unreachable, and the
 * symptom is a device that reports link up and counts packets in its own
 * registers while never completing a single DMA.
 *
 * The translation is described by the "dma-ranges" property of the PCI host
 * bridge in the device tree, which is where we read it from.  It is zero on
 * identity-mapped platforms (x86, most arm64 servers, and Raspberry Pi 4 with
 * 3GB or less of RAM), where everything below is a no-op.
 */
