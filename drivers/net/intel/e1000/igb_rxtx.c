 ...
 *
 * On SoCs whose PCIe host bridge is not cache coherent (the device tree bus
 * lacks "dma-coherent", e.g. the BCM2711 on Raspberry Pi 4 / Compute Module 4)
 * a bus-master read by the NIC can observe stale memory: data the CPU has just
 * stored is not yet visible to the device.  The Linux DMA API hides this by
 * cleaning the CPU data cache over a buffer before handing it to the device
 * (arch_sync_dma_for_device), but DPDK writes descriptors and packet data
 * straight into hugepage memory and rings the doorbell immediately.  The NIC
 * then fetches the previous contents -- zeroed descriptors with DD set and
 * null buffer addresses -- so transmits never complete and received data is
 * DMA'd to address zero.
 *
 * Mirror the kernel: clean the D-cache to the point of completion (DC CVAC,
 * as arm64 __dma_clean_area does) over every descriptor and packet buffer
 * before the tail register tells the NIC it may read them, and discard cached
 * copies (DC CIVAC, as __dma_inv_area does) of descriptor status and received
 * data before reading what the NIC wrote.
 *
 ...
 * per 64-byte line, flushing a freshly written descriptor would also restore
 * stale status for its neighbours and erase the DD bits the NIC just set (the
